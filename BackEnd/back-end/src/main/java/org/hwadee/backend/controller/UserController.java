...
import org.hwadee.backend.service.UserService;
import org.hwadee.backend.utils.MD5Utils;
import org.hwadee.backend.utils.PageUtils;
import org.hwadee.backend.utils.Query;
import org.hwadee.backend.utils.ResultMsg;
...
public class UserController {
    ...
    public ResultMsg edit(@RequestBody UserEntity user) {
        if(!(user.getUserId()!=0 && StringUtils.hasText(user.getName()) && StringUtils.hasText(user.getPhone()))){
    ...
    public ResultMsg add(@RequestBody UserEntity user) {
        if (!(StringUtils.hasText(user.getName()) && StringUtils.hasText(user.getPhone()) &&
                StringUtils.hasText(user.getAccount()) && StringUtils.hasText(user.getPassword()))){
            return ResultMsg.error("请求参数异常");
        }
        //验证账号名称是否存在
